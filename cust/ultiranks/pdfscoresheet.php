...
class PDF extends tFPDF_CellFit implements BundledPlayerListScoreSheet
{
    ...
    public function PrintScoreSheet($seasonname, $gameId, $hometeamname, $visitorteamname, $poolname, $time, $placename, $homeplayers = [], $visitorplayers = [])
    {
        ...
        $hometeaminfo = !empty($hometeamid) ? TeamInfo($hometeamid) : [];
        $visitorteaminfo = !empty($visitorteamid) ? TeamInfo($visitorteamid) : [];
        ...
        if (is_file($this->eventLogoLg)) {
            $this->Image($this->eventLogoLg, 140, 249, 40, 0);
        }
    ...
    public function SigFinalScore()
    {
        ...
        $this->SetFillColor(255);
        $this->SetFont('Arial', '', 10);
        $this->CellFitScale(30, 10, $this->homefullname, 'LRTB', 0, 'L', true);
        $this->Cell(18, 10, "", 'LRTB', 0, 'L', true);
        $this->Cell(18, 10, "", 'LRTB', 0, 'L', true);
        $this->Cell(40, 10, "", 'LRTB', 0, 'L', true);
        $this->Cell(12, 10, "", 'LRTB', 0, 'L', true);
        $this->Ln();

        $this->SetX($curX);

        //visitor grids
        $this->CellFitScale(30, 10, $this->visitorfullname, 'LRTB', 0, 'L', true);
        $this->Cell(18, 10, "", 'LRTB', 0, 'L', true);
        $this->Cell(18, 10, "", 'LRTB', 0, 'L', true);
        $this->Cell(40, 10, "", 'LRTB', 0, 'L', true);
        $this->Cell(12, 10, "", 'LRTB', 0, 'L', true);
    ...
    public function DefenseGridHeader()
    {
        $this->SetFont('Arial', '', 10);
    ...
    public function SplitOneCellTable($header, $data)
    {
        //header
        $this->SetFont('Arial', 'B', 12);
        $this->SetTextColor(0);
    ...
    public function SplitOneCellTable2($header, $data)
    {
        //header
        $this->SetFont('Arial', 'B', 12);
        $this->SetTextColor(0);
    ...
    public function SplitOneCellTable3($header, $data, $w1, $w2)
    {
        //header
        $this->SetFont('Arial', 'B', 10);
        $this->SetTextColor(0);
    ...
    public function ThreeCellTable($header1, $header2, $header3, $data1, $data2, $data3)
    {
        //header
        $this->SetFont('Arial', 'B', 12);
        $this->SetTextColor(0);
    ...
    public function SpiritHeader($yourteam, $opponent)
    {
    ...
    public function SpiritTable()
    {
        ...
        $i = 1;
        foreach ([
            ["Rules Knowledge and Use", "They did not purposefully misinterpret the rules. They kept to time limits. When they didn't know the rules, they showed a real willingness to learn."],
            ["Fouls and Body Contact","They avoided fouling, contact, and dangerous plays. They played safely. The game flowed smoothly."],
            ["Fair-Mindedness","They apologized in situations where it was appropriate, informed teammates about wrong/unnecessary calls. Only called significant breaches."],
            ["Attitude and Self-Control","They were polite. They played with appropriate intensity irrespective of the score. They left an overall positive impression during and after the game."],
            ["Communication","They communicated respectfully. They listened. They kept discussion to reasonable limits. They got to know us. They used hand signals."],
        ] as $s) {
            $curX = $this->GetX();
            $curY = $this->GetY();
