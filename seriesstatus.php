...
foreach ($teams as $team) {
    $statsRow = isset($teamStatsPoints[$team['team_id']]) ? $teamStatsPoints[$team['team_id']] : [
        "games" => 0,
        "wins" => 0,
        "draws" => 0,
        "losses" => 0,
...
if (ShowSpiritScoresForSeason($seasoninfo)) {
    ...
    if (!empty($spiritAvg)) {
        uasort($spiritAvg, function ($a, $b) {
            $va = isset($a['total']) ? $a['total'] : null;
            $vb = isset($b['total']) ? $b['total'] : null;
            if ($va == $vb) {
                return 0;
            }
            if ($va === null) {
